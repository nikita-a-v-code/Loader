...
const TTandTN = ({
  ...
  const [transformPoints, setTransformPoints] = React.useState(() => {
    ...
    for (let i = 0; i < pointsCount; i++) {
      points.push({
        /* Данные ТТ */
        showTransform: transformData[i]?.showTransform === true,
      ...
      <Box sx={{ display: "flex", flexDirection: "row", gap: 4, flexWrap: "wrap" }}>
        {transformPoints.map((point, index) => (
          ...
          <Box key={index} sx={{ mb: 2, border: "3px solid black", borderRadius: 2, p: 2, width: "100%" }}>
            ...
            {/* Переключатель для показа/скрытия трансформаторов */}
            <FormControlLabel
              control={
                <Switch
                  checked={point.showTransform === true}
                  onChange={(e) => handleFieldChange(index, "showTransform", e.target.checked)}
                  color="primary"
                />
              }
              label={
